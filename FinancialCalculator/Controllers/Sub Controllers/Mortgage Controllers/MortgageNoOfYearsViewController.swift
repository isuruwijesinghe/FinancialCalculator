//
//  MortgageNoOfYearsViewController.swift
//  FinancialCalculator
//
...
class MortgageNoOfYearsViewController: ParentViewController {
      ...
      @IBOutlet weak var tf_NoOfYears: UITextField!
      ...
      override func keyboardKeyPressed(value: String) {
          ...
          }else if (tf_NoOfYears.isFirstResponder){
              selectedText = tf_NoOfYears
              unit = "noOfPayments"
      ...
      func updateFields(){
          ...
          if (tf_LoanAmount.text != "0" && tf_Interest.text != "0"  && tf_Payment.text != "0") {
            ...
            //              let noOfPayments = (tf_NoOfYears.text! as NSString).doubleValue
            let compoundsPerYear = 12.00 //compounds per year is 12 by default cuz year has 12 months
            
            let value : Double = Calculations.calMortgageNumberOfPaymentsValue(PMT: payment, R: interest, P: presentValue, t: compoundsPerYear)
              
              tf_NoOfYears.text = String(format:"%.0f", value)
              
              let redTFColor = UIColor.red
              let greenTFColor = UIColor.green
              
              if tf_NoOfYears.text == "0" {
                  tf_NoOfYears.layer.borderColor = redTFColor.cgColor
                  tf_NoOfYears.layer.borderWidth = 1.0
              } else{
                  tf_NoOfYears.layer.borderColor = greenTFColor.cgColor
                  tf_NoOfYears.layer.borderWidth = 1.0
              }
      ...
      func savingItem() -> String{
          return "Loan value = Rs. " + String(format: "%.2f", Double(tf_LoanAmount.text!)!) + ", Interest = " + tf_Interest.text! + "%" + ", No. of years = " + String(format: "%.0f", Double(tf_NoOfYears.text!)!)  + ", Payment = Rs. " + String(format: "%.2f", Double(tf_Payment.text!)!)
      }
      ...
      func saveTfDataAppClose(){
          ...
          defaults.set(tf_NoOfYears.text, forKey: "mortgage_noOfMonths")
      ...
      func getDataWhenReopen(){
          ...
          tf_NoOfYears.text = defaults.string(forKey: "mortgage_noOfMonths")
