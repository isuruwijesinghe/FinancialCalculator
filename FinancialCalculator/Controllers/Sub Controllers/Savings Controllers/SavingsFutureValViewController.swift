//
//  SavingsFutureValViewController.swift
//  FinancialCalculator
//
...
class SavingsFutureValViewController: ParentViewController {
    ...
    override func keyboardKeyPressed(value: String) {
        ...
        }else if (tf_PresentValue.isFirstResponder){
    ...
    func updateFields(){
        ...
        if (tf_Interest.text != "0" && tf_PresentValue.text != "0"  && tf_NoOfPayments.text != "0" ) {
            ...
            let value: Double = Calculations.calSavingsFutureValue(P: presentValue, R: interest, t: noOfPayments)
    ...
    func savingItem() -> String{
        return ", Future value = Rs. " + String(format: "%.2f", Double(tf_FutureValue.text!)!) + ", Interest = " + tf_Interest.text! + "%" + ", No. of payments (years) = " + String(format: "%.2f", Double(tf_NoOfPayments.text!)!)  + ", Present Value = Rs. " + String(format: "%.2f", Double(tf_PresentValue.text!)!)
    }
    ...
    func saveTfDataAppClose(){
        ...
        defaults.set(tf_PresentValue.text, forKey: "savings_payment")
